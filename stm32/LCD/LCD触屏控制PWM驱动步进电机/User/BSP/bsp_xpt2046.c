...
void Calibrate_or_Get_TouchParaWithFlash(uint8_t LCD_Mode,uint8_t forceCal)
{
	...
	//读取触摸参数标志
	Flash_Read_Data(FLASH_TOUCH_PARA_FLAG_ADDR,&para_flag,1);
	...
	{ 		
		...
		if(  para_flag == FLASH_TOUCH_PARA_FLAG_VALUE && forceCal == 1)
		{
			Flash_Read_Data(FLASH_TOUCH_PARA_ADDR,(uint8_t *)&strXPT2046_TouchPara,4*6*8);	
		}
		...
		//写入触摸参数标志
		Flash_Buffer_Write(FLASH_TOUCH_PARA_FLAG_ADDR,&para_flag,1);
		//写入最新的触摸参数
		Flash_Buffer_Write(FLASH_TOUCH_PARA_ADDR,(uint8_t *)&strXPT2046_TouchPara,4*6*8);
...
void XPT2046_TouchUp(strType_XPT2046_Coordinate * touch) 
{
	...
	if(onoff>1)
	{
		...
		ILI9341_DispString_EN_CH(40+32,LINE(0),"开");
	}else
	{
		TIM_Cmd(ADVANCE_TIM, DISABLE);
		// 主输出使能，当使用的是通用定时器时，这句不需要
		TIM_CtrlPWMOutputs(ADVANCE_TIM, DISABLE);
		ILI9341_DispString_EN_CH(40+32,LINE(0),"关");
	}
	if(dir>1)
	{
		GPIO_SetBits(SDC_GPIO_PORT,SDC_DIR_GPIO_Pin);
		ILI9341_DispString_EN_CH(40+32+56+8,LINE(0),"正");
	}
	else
	{
		GPIO_ResetBits(SDC_GPIO_PORT,SDC_DIR_GPIO_Pin);
		ILI9341_DispString_EN_CH(40+32+56+8,LINE(0),"反");
	}
	if(en>1)
	{
		GPIO_SetBits(SDC_GPIO_PORT,SDC_EN_GPIO_Pin);
		ILI9341_DispString_EN_CH(40+32+56+56+16,LINE(0),"开");
	}
	else
	{
		GPIO_ResetBits(SDC_GPIO_PORT,SDC_EN_GPIO_Pin);
		ILI9341_DispString_EN_CH(40+32+56+56+16,LINE(0),"关");
	}
	//进行延时防止高频连按
	delay(0xfffff);
	/***在上面编写自己的触摸释放处理应用***/
