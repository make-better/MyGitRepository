...
void let_us_try()
{
	...
	ILI9341_DispStringLine_EN_CH(LINE(3),"第二句话");
  ILI9341_DispString_CH(0,LINE(4),"你好世界！");
	LCD_SetTextColor(RED);
  ILI9341_DispStringLine_EN_CH(LINE(5),"第三句话");
	ILI9341_DispString_CH(0,LINE(6),"你好世界！");
	ILI9341_DispStringLine_EN( LINE(7),"Hello World!");
	LCD_SetTextColor(GREEN);
	ILI9341_DispStringLine_EN_CH(LINE(8),"下一个");
  ILI9341_DispString_CH(0,LINE(9),"你好世界！");
	ILI9341_DisplayStringEx(0, 		//字符显示位置x
																 LINE(10), 				//字符显示位置y
																 32,	//要显示的字体宽度，英文字符在此基础上/2。注意为偶数
																 32,	//要显示的字体高度，注意为偶数
																 (uint8_t *)"      完了",					//显示的字符内容
																 0);  //是否反色显示
}
