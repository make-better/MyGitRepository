...
//EEPROM存储字符串
void EEPROM_STRING_WRITE(void) 
{
	uint16_t i;
	uint16_t str_byte = 0;     //存储字符串大小
	uint8_t write_buf[STR_SIZE];
  uint8_t read_buf[STR_SIZE];
	gets((char *)write_buf);   //从串口获取字符串
  str_byte = strlen((char*)write_buf);
	//write_buf[str_byte--] = '\0';
/*-----------------------------------------------------------------------------------*/  
  if (ee_CheckOk() == 0)
	{
		/* 没有检测到EEPROM */
  ...
  if (ee_WriteBytes(write_buf, 0, str_byte) == 0)
	{
		printf("写eeprom出错！\r\n");
	}
	else
  ...
  ee_Delay(0x0FFFFF);
/*-----------------------------------------------------------------------------------*/
  if (ee_ReadBytes(read_buf, 0, str_byte) == 0)
	{
		printf("读eeprom出错！\r\n");
	}
	else
